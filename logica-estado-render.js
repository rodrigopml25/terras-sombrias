...
function construirRolagemDanoArma(p, item, opts) {
  ...
  // "Auxílio Elementar — Fogo" (Soldado Elementar): +1d6 de Dano nos 2
  // próximos ataques com Arma — consome 1 carga por rolagem de Dano (ver
  // escolherAuxilioElementar/aplicarResetDeTurno, que zera cargas não
  // usadas no início do turno seguinte).
  if (p.auxilioElementarFogoCargas > 0) {
    const afRoll = 1 + Math.floor(Math.random() * 6);
    terms.push({ sign: '+', node: { type: 'dice', sides: 6, count: 1, results: [afRoll], sum: afRoll, countNode: null, label: 'Auxílio Elementar (Fogo)' } });
    total += afRoll;
    p.auxilioElementarFogoCargas--;
  }

  // "Encantamento Flamejante" (Soldado Elementar): +1d6 de Dano (atravessa
  // Armadura, mas o app não separa Dano de Armadura/Vida nas rolagens de
  // Arma — soma junto igual a qualquer outro bônus) em TODOS os ataques com
  // Arma até o final do turno — sem limite de usos (diferente do Auxílio
  // Elementar acima), então não consome nada aqui, só lê a marca (ver
  // aplicarResetDeTurno/resetLuta, que a desligam).
  if (p.encantamentoFlamejanteAtivo) {
    const efRoll = 1 + Math.floor(Math.random() * 6);
    terms.push({ sign: '+', node: { type: 'dice', sides: 6, count: 1, results: [efRoll], sum: efRoll, countNode: null, label: '🔥 Encantamento Flamejante' } });
    total += efRoll;
  }
...
function useSkill(pid, skid) {
  ...
  // "Aura de Fênix" (Soldado Elementar): não "acerta" nada (ver
  // HABILIDADES_SEM_ACERTO) — puramente narrativa (Passos/Vantagem em
  // ataques de longo alcance não são automatizados), então só liga a marca
  // pro badge aparecer (mesmo padrão simples de Fúria/Trovoada). Termina
  // sozinha no fim da Luta (ver resetLuta) ou manualmente (ver
  ...
  // "Auxílio Elementar" (Soldado Elementar): não "acerta" nada (ver
  // HABILIDADES_SEM_ACERTO) — abre a escolha entre os 4 elementos (ver
  // abrirAuxilioElementarModal/escolherAuxilioElementar). Só o Fogo é
  // automatizado (+1d6 de Dano nos 2 próximos ataques com Arma — ver
  // construirRolagemDanoArma); Ar/Gelo/Terra ficam narrativos, resolvidos
  // na mesa, igual à opção "limpar" de Honra.
  if (sk.id === 'sk_banco_soldado_elementar_auxilio_elementar' || sk.bancoId === 'soldado_elementar_auxilio_elementar') {
    abrirAuxilioElementarModal(pid);
    return;
  }
  ...
  // "Encantamento Flamejante" (Soldado Elementar): não "acerta" nada (ver
  // HABILIDADES_SEM_ACERTO) — liga a marca direto (sem modal, mesmo padrão
  // de Fúria/Trovoada): TODOS os ataques com Arma até o final do turno
  // ganham +1d6 de Dano que atravessa Armadura (somado de verdade em
  // construirRolagemDanoArma). Sem limite de usos dentro do turno (diferente
  // de Auxílio Elementar — Fogo, que tem só 2 cargas). Limpa em
  // aplicarResetDeTurno/resetLuta.
  if (sk.id === 'sk_banco_soldado_elementar_encantamento_flamejante' || sk.bancoId === 'soldado_elementar_encantamento_flamejante') {
...
function aplicarResetDeTurno() {
  ...
  PLAYERS.forEach(p => {
    ...
    // "Encantamento Flamejante" (Soldado Elementar): o +1d6 de Dano em
    // todos os ataques vale só até o final do turno em que foi usada.
    ...
    <div class="j-sidebar">
      <div class="j-id-card">
        ...
        <div style="display:flex;align-items:center;gap:8px;background:var(--red-bg);border:1px solid var(--red-bd);border-radius:10px;padding:8px 12px;margin-top:10px">
          <span style="font-size:18px">🔥</span>
          <div style="flex:1">
            <div style="font-size:12px;font-weight:700;color:#f08080">Aura de Fênix ativa</div>
            <div style="font-size:11px;color:var(--text2)">Asas de Fogo até o final da Luta/Cena · +10 de Passos e +1d6 de Vantagem em ataques de longo alcance enquanto voando</div>
          </div>
          <button onclick="event.stopPropagation();desativarAuraDeFenix(${p.id})" title="Encerrar a Aura de Fênix (ex: pousou)" style="background:none;border:1px solid var(--border2);color:var(--text2);width:22px;height:22px;border-radius:50%;cursor:pointer;font-size:11px;flex-shrink:0">✕</button>
        </div>` : ''}
        ${p.auxilioElementarFogoCargas > 0 ? `
        <div style="display:flex;align-items:center;gap:8px;background:var(--red-bg);border:1px solid var(--red-bd);border-radius:10px;padding:8px 12px;margin-top:10px">
          <span style="font-size:18px">🔥</span>
          <div style="flex:1">
            <div style="font-size:12px;font-weight:700;color:#f08080">Auxílio Elementar (Fogo): ${p.auxilioElementarFogoCargas} ataque${p.auxilioElementarFogoCargas > 1 ? 's' : ''} restante${p.auxilioElementarFogoCargas > 1 ? 's' : ''}</div>
            <div style="font-size:11px;color:var(--text2)">+1d6 de Dano no próximo ataque com Arma — consumido automaticamente</div>
          </div>
        </div>` : ''}
        ${p.encantamentoFlamejanteAtivo ? `
        <div style="display:flex;align-items:center;gap:8px;background:var(--red-bg);border:1px solid var(--red-bd);border-radius:10px;padding:8px 12px;margin-top:10px">
          <span style="font-size:18px">🔥</span>
          <div style="flex:1">
